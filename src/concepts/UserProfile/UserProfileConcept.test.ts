...
Deno.test("UserProfile Concept", async (t) => {
  ...
  await t.step("Action: createProfile", async (t) => {
    await t.step("should create a profile successfully", async () => {
      console.log(
        "Trace: Attempting to create profile for Alice in Baker House.",
      );
      const result = await userProfileConcept.createProfile({
        user: userAlice,
        displayName: "Alice",
        dorm: "Baker House",
      });
      ...
      const profileData = await userProfileConcept._getProfile({
        user: userAlice,
      });
    ...
    await t.step(
      "should fail if user already has a profile (requires)",
      async () => {
        console.log(
          "Trace: Attempting to create a duplicate profile for Alice.",
        );
        const result = await userProfileConcept.createProfile({
          user: userAlice,
    ...
    await t.step(
      "should fail for an invalid dorm name (requires)",
      async () => {
        console.log(
          "Trace: Attempting to create a profile for Bob with an invalid dorm.",
        );
        const result = await userProfileConcept.createProfile({
          user: userBob,
          displayName: "Bob",
        ...
        assertEquals(
          "error" in result && result.error,
          "Invalid dorm name: Invalid Dorm.",
        );
      },
    );
  });
  ...
  await t.step("Action: updateProfile", async (t) => {
    await t.step("should update an existing profile successfully", async () => {
      ...
      const result = await userProfileConcept.updateProfile({
        user: userAlice,
    ...
    await t.step(
      "should fail if user profile does not exist (requires)",
      async () => {
        console.log(
          "Trace: Attempting to update non-existent profile for Bob.",
        );
        const result = await userProfileConcept.updateProfile({
          user: userBob,
          displayName: "Bob",
  ...
  await t.step("Action: updateScores (system)", async (t) => {
    await t.step("should update scores for an existing user", async () => {
      ...
      const result = await userProfileConcept.updateScores({
        user: userAlice,
        lenderScore: 10,
        borrowerScore: 5,
      });
    ...
    await t.step(
      ...
      async () => {
        console.log(
          "Trace: Attempting to update scores for non-existent profile for Bob.",
        );
        const result = await userProfileConcept.updateScores({
          user: userBob,
          lenderScore: 10,
  ...
  await t.step("Principle Test", async () => {
    ...
    console.log(
      "Goal: If a user creates a profile, other users can find them by dorm and see their display name.",
    );
    ...
    const simmonsUsers = await userProfileConcept._getUsersByDorm({
      dorm: "Simmons Hall",
    });
    assertEquals(simmonsUsers.length, 2);
    const simmonsUserIds = simmonsUsers.map((u) => u.user);
    assertNotEquals(
      simmonsUserIds.indexOf(userAlice),
      -1,
      "Alice should be in Simmons Hall",
    );
    assertNotEquals(
      simmonsUserIds.indexOf(userBob),
      -1,
      "Bob should be in Simmons Hall",
    );
    assertEquals(
      simmonsUserIds.indexOf(userCharlie),
      -1,
      "Charlie should not be in Simmons Hall",
    );

    console.log(
      "Verification 2: Query for a user's profile to see their display name.",
    );
    const bobsProfile = await userProfileConcept._getProfile({ user: userBob });
    assertEquals(bobsProfile[0]?.profile.displayName, "Bob");

    console.log(
      "Principle fulfilled: Users can be found by dorm, and their display names are visible.",
    );
